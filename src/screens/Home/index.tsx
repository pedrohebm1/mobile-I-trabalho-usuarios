...
export function Home() {
  const acceptableCities = ["BM", "VR", "BP", "RJ", "SP", "PH"];
  ...
  function handleAddNewUser() {
    ...
    if (!acceptableCities.includes(city.trim())) {
      return Alert.alert("Usuário", "Favor inserir uma cidade válida");
    }
  ...
  function showTotalUser() {
    ...
    console.log(`Total de usuários que moram em VR: ${users.filter((user)=>user.city==="VR").length}`);
    console.log(`Total de usuários que moram em BM e PH: ${users.filter((user)=>(user.city==="BM" || user.city==="PH")).length}`);
    console.log(`Total de usuários que não moram em VR: ${users.filter((user)=>user.city!=="VR").length}`);
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.button} onPress={showTotalUser}>
